...
--
-- Host: 127.0.0.1
-- Generation Time: May 06, 2025 at 12:35 PM
...
-- Functions
--
CREATE DEFINER=`root`@`localhost` FUNCTION `get_average_order_amount` (`cid` INT) RETURNS DECIMAL(10,2) DETERMINISTIC READS SQL DATA BEGIN
    ...
    RETURN IFNULL(avg_amount, 0.00);
END$$

CREATE DEFINER=`root`@`localhost` FUNCTION `get_customer_order_count` (`cid` INT) RETURNS INT(11) DETERMINISTIC READS SQL DATA BEGIN
    DECLARE order_count INT;
    SELECT COUNT(*) INTO order_count FROM orders WHERE customer_id = cid;
    RETURN order_count;
END$$

CREATE DEFINER=`root`@`localhost` FUNCTION `get_product_stock_status` (`pid` INT) RETURNS VARCHAR(20) CHARSET utf8mb4 COLLATE utf8mb4_general_ci DETERMINISTIC READS SQL DATA BEGIN
    DECLARE stock INT;
    SELECT stock_quantity INTO stock FROM products WHERE product_id = pid;
...
CREATE DEFINER=`root`@`localhost` FUNCTION `get_shipping_delay_days` (`oid` INT) RETURNS INT(11) DETERMINISTIC READS SQL DATA BEGIN
    DECLARE delay INT;
    ...
    RETURN IFNULL(delay, -1); -- Returns -1 if dates are NULL
END$$

CREATE DEFINER=`root`@`localhost` FUNCTION `get_total_spent_by_customer` (`cid` INT) RETURNS DECIMAL(10,2) DETERMINISTIC READS SQL DATA BEGIN
